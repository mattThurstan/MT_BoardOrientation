...
class MT_BoardOrientation
{
  ...
  private:
	...
	float indGetAverage();
	float _indThreshold;					//roll deadzone. when to turn on left/right leaning/turning indicators.
  ...
  public:
	...
	void AddToAverage(float accelY, float roll);	//call this something like '_diSize' times per second (currently 10). this would give a rolling average of 10 per sec.
